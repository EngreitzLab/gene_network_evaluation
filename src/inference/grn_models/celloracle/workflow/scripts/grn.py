...
parser.add_argument('-b','--path_base_grn', required=True, type=str, help="path to base GRN containing initial TF to gene links. Will ignore r2g and tf2r if provided.")
...
parser.add_argument('-s','--scale', required=False, type=str, default=True, help="whether to scale data before regression")
...
path_base_grn = args['path_base_grn']
...
# Process base GRN
if path_base_grn:
    print("Loading base GRN from file, not recommended if you have scATAC-seq data.")
    base_grn = pd.read_parquet(path_base_grn)
else:
    print("Building base GRN from r2g and tf2r.")
...
if layer in adata.layers:
    ...
    if "counts" not in adata.layers:
        raise ValueError("Could not find counts layer in adata.")
...
for cluster in adata.obs[cluster_key].cat.categories:
    ...
    net = co.Net(
        ...
        gem_standerdized=None, # We want it to calculate the standardized gene expression matrix internally
