...
- Runs Latent Dirichlet Allocation (LDA) on the binarized scATAC-seq matrix
    - Uses collapsed Gibbs sampling (cgs) from **`lda`** Python package for its LDA implementation (https://lda.readthedocs.io/en/latest/#)
        - Parallelizable but this can lead to memory issues -- **currently hardcoding to 1 cpu**
    ```python
    import lda
    model = lda.LDA(
                n_topics=n_topics,
...
SCENIC+ uses the cisTopic object to identify regions for motif enrichment in three ways:
    1. Calls regions for each topic using the Otsu method
    2. Calls regions for each topic using the top N regions based on the region-topic probabilities
    3. Calls regions using differentially accessible regions
        ...
        - Finds highly variable regions using a ScanPy-like method to find highly variable regions 
...
- Saves all of these as "region sets" in dictionary a pickle file (keys are 'topics_otsu', 'topics_top', 'DARs')
...
```
![Alt text](motif_enr.png)
SCENIC+ uses two strategies to infer TF motif enrichment to build cistromes (a TF and all it's putative enhancers). They require several databases of external information: 
    - A pre-built database of TF motifs and their annotations (https://pycistarget.readthedocs.io/en/latest/motif_table.html): We use the SCENIC+ motif collection that includes more than 34,524 unique motifs gathered from 29 motif collections --> Currently found in `smk/resources/annotations/human/motifs-v10nr_clust-nr.hgnc-m0.001-o0.0.tbl` (TODO: need to add how to download)
    - Requires a pre-built database of **scores** of genomic loci for each TF motif (https://pycistarget.readthedocs.io/en/latest/tools.html#generation-of-cistarget-databases): We use the scores computed at ENCODE SCREEN loci --> `smk/resources/scores/human/hg38_screen_v10_clust.regions_vs_motifs.scores.feather`
    - A pre-built database of **rankings** of genomic loci for each TF motif (https://pycistarget.readthedocs.io/en/latest/tools.html#generation-of-cistarget-databases): We use the rankings computed at ENCODE SCREEN loci --> `smk/resources/rankings/human/hg38_screen_v10_clust.regions_vs_motifs.rankings.feather`
    ![Alt text](databases.png)
1. cisTarget
    ![Alt text](cistarget.png)
...
```
SCENIC+ uses gradient boosted models to calculate edges from regions to genes (e2g links) and TFs to genes (a classic GRN):
1. Calculates TF-gene links usis gradient-boosting machine regression by predicting raw TF expression from raw gene expression counts
    ...
    - Importance score of a TF for itself was set to the maximum importance score across all genes added with an arbitrary small value of 1 × 10−5.
2. Calculates e2g links using gradient-boosting machine regression by predicting TF expression from imputed region accessibility
    - Define a search space as minimum 1kb and maximum 150kb from the TSS (+/- 10bp) of the gene (both upstream and downstream) THIS CAN BE MODIFIED
    - After the model is fit, the importance score of each feature (region) is calculated and considered as a putative e2g link
    - Binarized by taking the 85th, 90th and 95th quantile of the region-to-gene importance scores, the top 5, 10 and 15 regions per gene and a custom BASC implementation (what is this???)
3. Builds an eRegulon (a TF with its set of target regions and genes) for each TF by combining the e2g links, cistromes and TF-gene links
    ![Alt text](gsea.png)
